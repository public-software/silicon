...
This is `silicon`, one repository of [Public Software](https://publicsoftware.dev): a spec-first cleanroom Rust suite built by people and coding agents together. Read this file before the README; it is the contract every agent works under here.

**Ring:** system · **Layers:** L0 · **Wave:** 3 · Open chip designs: root of trust, RF front-end control, FPGA GPU, reference RISC-V platform definitions; shuttle submissions. Planned components: rot chip · rf-fe · fpga-gpu · riscv-platform · shuttle/ runs.
...
- A new crate is `pub new <kind> <name>` with a kind from lib, app, service, plugin, spec; it lands in `crates/pub-silicon-<name>/` and gets a `[[component]]` entry in `CATALOG.toml`.

## Conventions

- Crates are named `pub-silicon-<name>`; a service is the binary `pubd-<name>`; WIT interfaces live under `public:`.
...
- The merge gates decide (`pub check`, CI, the agent review `suite / policy`, the maintainer team `public-software/maint-silicon`), not who or what wrote the change.
<!-- agents:end -->

## Repo notes

_Maintainers of `silicon` write here what the generated section cannot know: how the crates fit together, how to run the thing, which specifications matter, what not to touch. Everything above this heading is regenerated._
